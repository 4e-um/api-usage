...
public class PlanChangeUtil {
    ...
    public List<CalculatedLimitSchema> calculate(List<PlanChangeSchema> events) {
        ...
        for (PlanChangeSchema event : events) {
            yyyyMM = toYyyyMM(event.changedAt().toString());

            processedKey = "processed:plan:" + yyyyMM + ":" + event.subscriptionId();
            added = redisTemplate.opsForSet()
                    .add(processedKey, event.eventId());
            ...
            if ("ULTIMATE".equals(event.unit())) {
                ...
                finalLimit = getMonthFinalLimit(yyyyMM, event, prevUnit);
            }

            long ttlSec = UsageTimeUtil
                    .ttlToNextMonthWithBufferSec(event.changedAt().toString(), 2);

            results.add(new CalculatedLimitSchema(
    ...
    private long getMonthFinalLimit(String yyyyMM, PlanChangeSchema event, String prevUnit) {
        OffsetDateTime kstTime = event.changedAt()
        ...
        String limitKey = "limit:" + yyyyMM + ":" + event.subscriptionId();
